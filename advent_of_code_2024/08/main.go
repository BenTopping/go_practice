	...
	for _, frequency := range frequencies {
		for _, node := range frequency {
			for n := 0; n < len(frequency); n++ {
				// If on the current node skip
				if frequency[n] == node {
					if len(frequency) > 1 { anti_nodes[node] = 1 }
					continue
				}
				...
				x_diff := node.X - anti_node.X
				y_diff := node.Y - anti_node.Y
				// Whilst in the bounds create a new anti node of the current node + difference
				for anti_node.X > 0 && anti_node.X < max_x && anti_node.Y > 0 && anti_node.Y < y_pos {
					anti_nodes[anti_node] = 1
					anti_node.X = anti_node.X - x_diff
					anti_node.Y = anti_node.Y - y_diff
				}
